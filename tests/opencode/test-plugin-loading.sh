#!/usr/bin/env bash
# Test: Plugin Loading
# Verifies that the snowball plugin loads correctly in OpenCode
set -euo pipefail
...
plugin_link="$OPENCODE_CONFIG_DIR/plugins/snowball.js"
...
skill_count=$(find "$SNOWBALL_SKILLS_DIR" -name "SKILL.md" | wc -l)
if [ "$skill_count" -gt 0 ]; then
  echo "  [PASS] Found $skill_count skills"
else
  echo "  [FAIL] No skills found in $SNOWBALL_SKILLS_DIR"
  exit 1
fi

# Test 3: Check using-snowball skill exists (critical for bootstrap)
echo "Test 3: Checking using-snowball skill (required for bootstrap)..."
if [ -f "$SNOWBALL_SKILLS_DIR/using-snowball/SKILL.md" ]; then
  echo "  [PASS] using-snowball skill exists"
else
  echo "  [FAIL] using-snowball skill not found (required for bootstrap)"
  exit 1
fi

# Test 4: Verify plugin JavaScript syntax (basic check)
echo "Test 4: Checking plugin JavaScript syntax..."
if node --check "$SNOWBALL_PLUGIN_FILE" 2>/dev/null; then
...
if grep -q 'configDir}/skills/snowball/' "$SNOWBALL_PLUGIN_FILE"; then
