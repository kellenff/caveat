...
var exports_write_madr = {};
__export(exports_write_madr, {
  writeMadr: () => writeMadr,
  timestampPrefix: () => timestampPrefix,
  slugify: () => slugify,
  assembleMadr: () => assembleMadr,
  assembleFrontmatter: () => assembleFrontmatter,
  assembleBody: () => assembleBody
});
module.exports = __toCommonJS(exports_write_madr);
...
function assembleFrontmatter(input) {
  ...
  return dump(fm, { lineWidth: 120, noRefs: true });
}
...
function timestampPrefix(isoDate) {
  ...
  if (!m)
    throw new Error(`unparseable date: ${isoDate}`);
